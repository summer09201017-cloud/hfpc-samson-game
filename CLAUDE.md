...
- **隱藏結局(逃跑彩蛋)**:場地右下角有**隱藏出口**——牆照常畫完整(`ESCAPE` 缺口**不繪製、看不出來**),但 `samson._clamp(exitGap)` 在該角落放寬夾邊,參孫摸到右下角能**走出競技場**;越過邊界後再多走 `ESCAPE.out`(70px,走遠一點)→ `game.step` 觸發 `enterEscape()` → `STATE.ESCAPED`,顯示「🏃 逃跑雖可恥但有用」(`scripture.js` 的 `LEVEL1.escape`,輕鬆向但帶聖經「智慧避開」反思)。不算過關也不算失敗,清掉墮落、可重玩。嵌入模式四牆封閉、不觸發。
...
```
src/
  game.js      主迴圈 + 狀態機(title/intro/fight/reviving/finisher/badending/win/lose/escaped/paused)+ 戰鬥裁判 + 嵌入契約

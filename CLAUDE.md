...
- **隱藏結局(逃跑彩蛋)**:場地**右下角的牆開了一道缺口**(`ESCAPE` 64×56,renderer 把底牆右端、右牆底端畫成開口 + 門柱)。參孫可從缺口**走出競技場**:`samson._clamp(exitGap)` 在缺口帶放寬該道牆的夾邊;`game.step` 偵測參孫中心越過右牆或底牆 → `enterEscape()` → `STATE.ESCAPED`,顯示「🏃 逃跑雖可恥但有用」(`scripture.js` 的 `LEVEL1.escape`,輕鬆向但帶聖經「智慧避開」反思)。不算過關也不算失敗,清掉墮落、可重玩。嵌入模式四牆封閉、不觸發。

...
- **隱藏結局(逃跑彩蛋)**:場地**最右下角**有一塊 `ESCAPE`(70×60)**隱形熱區**(不繪製任何提示),走進去 → `enterEscape()` → `STATE.ESCAPED`,顯示「🏃 逃跑雖可恥但有用」(`scripture.js` 的 `LEVEL1.escape`,輕鬆向但帶聖經「智慧避開」反思)。不算過關也不算失敗,清掉墮落、可重玩。嵌入模式不觸發。
